...
void processCommands() {
    ...
    //VF("MSG: Command A="); V(command); VL(parameter); 
    }
#ifdef HAL_SERIAL_B_ENABLED
    ...
    //VF("MSG: Command B="); V(command); VL(parameter); 
    } 
#endif
#ifdef HAL_SERIAL_C_ENABLED
      ...
      strcpy(command,cmdX.getCmd()); 
      strcpy(parameter,cmdX.getParameter()); 
      cmdX.flush(); 
      process_command=COMMAND_SERIAL_X; 
      //VF("MSG: Command X="); V(command); VL(parameter); 
    }
...
//            The current scope altitude
      if (command[1] == 'A' && parameter[0] == 0)  { 
        getHor(&f,&f1); 
        doubleToDms(reply,&f,false,true,precision); boolReply=false; } else
...
//            Returns: n (numeric value, possibly floating point)//cmdSend(":hQ#", 0);
      if (command[1] == 'X')  {
...
//            Returns: DDD*MM# or DDD*MM'SS# (based on precision setting)
      if (command[1] == 'Z' && parameter[0] == 0)  { 
        getHor(&f,&f1); 
        f1=degRange(f1); 
        doubleToDms(reply,&f1,true,false,precision); 
        boolReply=false;
      } else commandError=CE_CMD_UNKNOWN;
    } else
      ...
      if (command[1] == 'X')  {
        ...
        if (parameter[0] == '0') { // 0n: Align Model
          ...
          switch (parameter[1]) {
            ...
            case 'D': { if (!dmsToDouble(&Align.mount[star].dec,&parameter[3],true,PM_HIGH))  commandError=CE_PARAM_FORM; else Align.mount[star].dec =Align.mount[star].dec/Rad;        } break; // Mount #n Dec
