...
class focuserDC : public focuser  {
  public:
    void init(int stepPin, int dirPin, int enPin, int nvAddress, float maxRate, double stepsPerMicro, double min, double max, double minRate) {
      ...
      dcMotor.init(stepPin,dirPin,enPin,2);
      ...
      VF("minRate="); VL(minRate);
      VF("maxRate="); VL(maxRate);
      VF("spm="); VL(spm);
      VF("powerFor1mmSec="); VL(powerFor1mmSec);
      VF("spos="); VL(spos);
      VF("target.part.m="); VL(target.part.m);
      VF("target.part.f="); VL(target.part.f);
      VF("target.fixed="); VL(target.fixed);
      VF("lastTarget="); VL(lastTarget);
      VF("spsMax="); VL(spsMax);
      VF("nextPhysicalMove="); VL(nextPhysicalMove);
      VF("smax="); VL(smax);
      VF("smin="); VL(smin);
    }
    ...
    bool setTarget(long pos) {
      ...
      target.part.m=pos; target.part.f=0;
       VF("pos_target.part.m="); VL(target.part.m);
    ...
    void relativeTarget(long pos) {
      dcMotor.setPower((moveRate/1000.0)*powerFor1mmSec);
      VF("RTpos ="); VL(pos);
      VF("rel_target.part.m="); VL(target.part.m);
      target.part.m+=pos; target.part.f=0;
       VF("rel_target.part.m="); VL(target.part.m);
      if ((long)target.part.m < smin) target.part.m=smin; if ((long)target.part.m > smax) target.part.m=smax;
       VF("rel_target.part.m="); VL(target.part.m);
       VF("delta.fixed="); VL(delta.fixed);
    }
